...
import { applyFilterRules, formatFilterRulesLog } from "@/lib/filterRules/index.js";
...
export async function handleChatCore({ body, modelInfo, credentials, log, onCredentialsRefreshed, onRequestSuccess, onDisconnect, clientRawRequest, connectionId, userAgent, apiKey, ccFilterNaming, rtkEnabled, headroomEnabled, headroomUrl, headroomCompressUserMessages, headroomTimeoutMs, cavemanEnabled, cavemanLevel, ponytailEnabled, ponytailLevel, pxpipeEnabled, pxpipeMinChars, pxpipeTimeoutMs, pxpipeTransform, onPxpipeEvent, sourceFormatOverride, providerThinking, filterRulesEnabled }) {
  ...
  // Filter rules: sanitize request content in-place before dispatch. Gated by
  // the same client opt-out header as the token savers; fail-open internally.
  if (tokenSaverEnabled && filterRulesEnabled) {
    const frStats = applyFilterRules(translatedBody, { enabled: true, format: finalFormat });
    const frLine = formatFilterRulesLog(frStats);
    if (frLine) log?.line?.(reqTag, "⚙", frLine);
  }
